...
<template>
    <el-row>
      <!--搜索区域-->
        ...
        <!--列表区域-->
        <el-col :span="24">
            <el-table :data="tenantList" border class="el-table-jt" @selection-change="handleChange">
        ...
        <el-col :span="24" class="el-table-page">
          ...
          <el-col :span="24" class="el-addRole">
              <addTenant ref="addTenant"></addTenant>
        </el-col>
    </el-row>
</template>
    ...
    methods: {
     ...
     onSearch (index) {
        ...
        if (index) {
		  	...
		  	self.$backend.request(self.$api.tenant.queryTenantBatch, this.searchInfo)
			  	.then((res) => {
			  	 self.total = res.data.total
                // self.searchInfo.pageNum = res.data.pageNum
                self.tenantList = res.data.list
				})
      },
      /**
		  * 重置
      ...
      /**
		   ...
		   */
        onAdd () {
           this.$refs.addTenant.showflag = true
        },
      /**
       *查看客户详情
       */
