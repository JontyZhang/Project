...
import devConfig from '../conf/config.dev'
...
function getUrl (api) {
    ...
    let serverFullPath = config.servers[server]
    let arr = serverFullPath.split(/:\d/)
    let result = ''
    if (arr && arr.length > 1) {
    	let prefix = arr[1].substring(arr[1].indexOf('/') + 1) // 获取ip端口后面的请求路径
    	result = prefix + api.path
    }
    return result
}
