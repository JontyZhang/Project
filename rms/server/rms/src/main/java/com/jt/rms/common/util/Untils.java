...
public class Untils {
	...
	* */
	 ...
	 * 生成合同编号
	 */
	public static String makeContractNO(String landlordCID,String tenantCID){
		Calendar now = Calendar.getInstance();
		String nowNYR=now.get(Calendar.YEAR)+(now.get(Calendar.MONTH) + 1)+now.get(Calendar.DAY_OF_MONTH)+"";
		String contractNO=nowNYR+landlordCID+tenantCID;
		return contractNO;
	}
	/**
	 * m5加密
	...
	public static void dateSort(List<Map<String,Object>> sortList,String cloum){
		...
		//SimpleDateFormat sd=new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
		 try {
			 if(!Untils.isEmpty(sortList)){
					for(int i=0 ,length=sortList.size()-1;i<length;i++){
						...
						for(int j=i+1,lengtht=sortList.size();j<lengtht;j++){
							...
							//resultMap.put(cloum, sd.format(DateFormatUtil.convertStringToDate(resultMap.get(cloum).toString())));
							//resultMapTemp.put(cloum, sd.format(DateFormatUtil.convertStringToDate(resultMapTemp.get(cloum).toString())));
			...
			logger.error("untils dateSort error",e);
		}
	}
	...
	public static void  gjDateSort(List<Map<String,Object>> sortList,String cloum,SimpleDateFormat sd) throws ParseException{
		...
		try {
			...
			logger.error("untils gjDateSort error",e);
		}
	}
